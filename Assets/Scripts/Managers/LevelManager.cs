...
public class LevelManager : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        levelController.OnPointsIncrease += points =>
        {
            levelUIManager.SetInGameScore( points );
        };
        levelController.OnLevelPass += () =>
        {
            // PlayerStatistics.IncreaseLevel();
            Debug.Log( $"Level must be increased: {PlayerStatistics.Level} -> {PlayerStatistics.Level + 1}" );
